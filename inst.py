...
def check_and_update_missing_strings_in_file(file_path, search_strings):
    updated_file_content = []
    with open(file_path, 'r') as file:
        for line_number, line in enumerate(file, start=1):
            missing_strings = []
            for search_string in search_strings:
                if not re.search(r'\b' + re.escape(search_string) + r'\b', line, flags=re.IGNORECASE):
                    missing_strings.append(search_string)
            if len(missing_strings) == len(search_strings):
                # Both strings are missing, update the line
                updated_line = re.sub(r'\b(input)\b', 'input wire', line, flags=re.IGNORECASE)
                updated_line = re.sub(r'\b(output)\b', 'output wire', updated_line, flags=re.IGNORECASE)
                updated_file_content.append(updated_line.strip())
            else:
                updated_file_content.append(line.strip())

    # Write the updated content back to the file
    with open(file_path, 'w') as file:
        file.write('\n'.join(updated_file_content))

file_path = 'module_add_wires.txt'
search_strings = [' wire ', ' reg ', 'module ']
check_and_update_missing_strings_in_file(filename, search_strings)


right_words = ['module']
with open(filename) as oldfile, open('module1.txt', 'w') as newfile:
    for line in oldfile:
        if any(bad_word in line for bad_word in right_words):
            newfile.write(line)

with open('module1.txt','r') as oldfile , open('module_name.txt', 'w') as newfile:
    while True:
...
with open('port_list_filter.txt','r') as oldfile , open('wire_list_new.txt', 'w') as newfile:
    while True:
        ...
        substituted_phrase = substituted_phrase.replace(",", ";" )
        # update or comment next two lines wor adding sufix to wires
        substituted_phrase = substituted_phrase.replace("_i", "" )
        substituted_phrase = substituted_phrase.replace("_o", "" )
...
with open('params4.txt', 'w') as newf:
    for line in port_lines:
        Len = len(line)
        print('     .' + line + " "*(maxparamLen+2-Len) + '( ' + line + " "*(2*maxparamLen-2*minparamLen-Len) + '),', file=newf)
...
if os.path.exists('clean.txt'):
    os.remove('clean.txt')
if os.path.exists('module1.txt'):
    os.remove('module1.txt')
...
if os.path.exists('test1.txt'):
    os.remove('test1.txt')
if os.path.exists('module_add_wires.txt'):
    os.remove('module_add_wires.txt')